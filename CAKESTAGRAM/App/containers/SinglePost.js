...
/********* FIN TEST *********/
import {postLikeRequest} from '../redux/reducers/likeReducer'
...
showComments = () => {
  ...
  const commentsShow =
    this.props.comments.length > 0 ? (
      <View>{this.displayComments()}</View>
    ) : (
      <Text />
    );
  return (
...
render() {
  ...
  const avatar = (
    <Avatar
      ...
      containerStyle={styles.singlePostAvatar}
    />
  );

  const uri = post ? post.thumb : imgBase;
  const image = (
    <Image source={{ uri }} style={{ height: 200 }} resizeMode="cover" />
  );
  ...
  const comment = post.comments_count ? (
    <Icon
      ...
      size={20}
      solid
      onPress={this.goToSinglePostDetail}
    />
  ) : (
    <Icon
      name="comment"
  ...
  return (
    <Card>
      ...
      {this.props.withComments ? this.showComments() : <Text />}
    </Card>
  );
}
}
...
mapDispatchToProps = dispatch => {
  return {
    ...
    postLikeRequest: ()=>dispatch(postLikeRequest())
  };
};
